...
> hidx 往返/损坏、重开载入)。
> **建图精度档位**(`BuildPrecision`,§4.4):默认 `Hybrid`——建图遍历用段内临时
> i8 码流近似距离、选邻/剪枝仍用 f32 精排(读带宽 ÷4);`Builder::build_precision(F32)`
> 恢复全 f32 精确建图(原行为)。临时码流不落盘,hidx 格式与档位解耦
> (`FC-INDEX-POST-010/011`)。
> **批内并行建图**:批内节点基于批开始图快照并行搜索/选邻、批间串行应用,批行数只
> 依赖节点数,结果与线程数无关(`FC-INDEX-POST-012`;`Builder::parallelism` 调线程数)。
...
**工程注**:上述是**单线程**界。Mneme 用 scoped threads 按批并行:每次构建生成一个
**常驻 worker 池**(计划/修剪任务经 channel 分派,主线程在两阶段之间应用),批内节点在
批开始图快照上并行搜索/选邻(只读),批间串行应用连边与修剪;批行数只依赖节点数、与
线程数无关,见 §4.4 与 `FC-INDEX-POST-012`。建库吞吐门槛为 4 核基准 ≥300 向量/秒
...
> 再把**修剪邻接计算并行求值**(只读批末快照、各改各的邻接表)、按序写回;并行执行器
> 为每次构建一个**常驻 worker 池**(`thread::scope` 一次生成,任务/结果经 channel
> 分派;计划与修剪阶段 worker 持共享读锁、主线程只在两阶段之间持写锁应用,绝不并发
> 读写),避免逐批创建/销毁线程;批行数
> 只依赖节点数(小图 ≤64 行退化串行、大图固定 128,首批受 `m0` 约束),线程数取
> `Builder::parallelism`(0 = 可用核数,上限 8),**结果与线程数无关**(跨线程数
...
> 配置(默认 128/64/8/4);flush 切块行数与块级并行经
...
>
> **建图精度档位**(`BuildPrecision`,`FC-INDEX-POST-010/011`):默认 `Hybrid` 档把
> 遍历期的距离计算从 f32 原向量改为**段内临时 i8 码流**(`src/quant/scalar_i8.rs`
> 的段级统计/编码,复用 `FC-QUANT-POST-001` 误差界;副本只在构建期存活、不落盘、
> 不改变 hidx 格式);`search_layer`/`greedy_query` 走 i8 近似,选邻前对候选按 f32
> 原向量**重排**(计划阶段完成),`select_neighbors`/`prune` 仍为 f32 精排。`F32` 档为
> 原全精确行为,经 `Builder::build_precision` 可选。两档构建均确定(同配置同输入
> 逐字节相同 hidx);两档距离的浮点求和路径不同(码流×权重乘加 vs 点积),不承诺
> 逐位同值,`Hybrid` 的图结构由 `FC-INDEX-POST-011` 的召回对照约束。
...
> **可变表(未落盘尾部)**:尚未 `flush` 的记录不在任何 HNSW 中,查询时作为一个
> "未覆盖尾部"直接暴力扫描(其规模通常远小于 `brute_force_max_rows`),与各段 TopK
> 一起归并——因此新写入无需 `flush` 即可被检索([04 §8](04-l2-persist.md))。
>
> **纯内存库的内存段**:纯内存库(无 `path`)的 `Mneme::flush` 用同一 `IndexFactory`
> 把未覆盖槽位**建成内存段**(不写 vsec/msec/hidx、不做量化副本,`quant = F32`),
> 此后查询走「内存段 ANN + 尾部暴力」,与持久库同图、同口径;无新增时为空操作
> (`FC-INDEX-INV-008`)。未 `flush` 的纯内存库仍是全量暴力扫描。
...
> 邻接按节点惰性解码缓存;构建路径 `GraphStore::Heap` 保持原有逐节点邻接表。
> 载入期仍做一次无分配的邻接结构校验(度数上界/邻居越界/自环/布局吻合,
> FC-INDEX-ERR-001),故与整段解码的校验强度一致。
