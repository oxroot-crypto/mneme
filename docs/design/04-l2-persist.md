...
每个文件头都带 `format_version`(本章 §2),MANIFEST 另记录各段的版本。当前只有唯一的
格式版本,不存在更早的已发布格式,规则从简:
...
- **版本号策略**:`format_version` 是 16 位,**高 8 位为主版本、低 8 位为次版本**
  ...
  仅新增可选字段/保留位 → 次版本 +1。**当前不维护兼容矩阵**:版本不同即拒绝,
  不保留"读旧开发格式"的读取分支、不在后台升级旧段、不支持混合版本库
  (见 `AGENTS.md`「项目状态与兼容纪律」)。
- **版本策略**:破坏性变更需在 `docs/spec/contracts.md` 登记弃用与迁移约束,
  再按该策略实现。
