# 09 测试与验收:如何证明以上不是纸上谈兵

> **本章目标**:把前 8 章与 [11](11-api-reference.md) 的每个"不变量(I1–I18)"和"性能承诺"
> 翻译成可执行的测试。
> **前置阅读**:各章"层边界契约"小节。
...
**总纲**:每章的"不变量"编号(I1–I18,见各章末尾与 [11 §9](11-api-reference.md))
...
| I12 | 量化模式 `Hit.score` 为 f32 精排分 | §4 量化收益基准 + §3.2 分数口径断言 |
...
**另**:L1 的"排序全等性"([03 §2.2](03-l1-memory.md))与 L3 的"`ef → ∞` 收敛"
([05 §12](05-l3-hnsw.md))虽未编号,但作为属性测试在 §3 一并断言。

---

## 2. 崩溃注入(I1–I4,I10,I15,I16 的验收)
...
陈旧锁:     模拟持锁进程死亡 → 再次 open 能自动接管而非永久 Busy(11 §3)
```
...
## 6. 长跑测试(I8–I11,I17 的验收)

```text
场景: 模拟 Agent 日常 —— 随机命名空间 20 个,持续写入(90% 短 TTL,
      10% 长期记忆)+ 周期检索 + 周期 retain + 每 2h 快照备份
时长: 24h(CI 每周), 断言:
