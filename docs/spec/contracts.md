...
| FC-PERSIST-INV-004 | INV | **I4**:WAL 总量 ≤ `wal_bytes`;段文件只增不改 | `tests/persist_contracts.rs::wal_capacity_triggers_incremental_flush`、`tests/persist_contracts.rs::committed_segment_is_write_once` | Passed |
...
| FC-QUANT-ERR-002 | ERR | 纯内存库(`Builder` 无 `path`)配置 `F16`/`I8Rescored` → 构造期 `Unsupported`;打开含 f16 段而当前构建未开 `quant-f16` → `Unsupported`,绝不静默按 f32 服务(跨 feature 端到端用例登记于设计 14 §7 CI 矩阵,单构建无法覆盖) | `tests/l6_contracts.rs::in_memory_quantization_is_unsupported`、`src/quant/mod.rs::format_support_matches_feature_gate` | Passed |
