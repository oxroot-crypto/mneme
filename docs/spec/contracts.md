...
| FC-PERSIST-POST-002 | POST | Checkpoint 仅当 `seqno ≤ watermark` 的覆盖条目已物化(写入增量段或 `delta` 区)时才截断/删除 WAL;WAL 轮转后按文件序回放、`seqno ≤ watermark` 的帧跳过,单批不跨文件(FC-PERSIST-POST-011) | `tests/persist_contracts.rs::flush_checkpoints_after_materialize` | Passed |
...
| FC-PERSIST-ERR-011 | ERR | msec `delta` 区结构畸形(kind 未知、长度越界或相加溢出、条目区 CRC 不符、区尾残留)→ `Corrupted`,绝不 panic / 回绕 / 静默截断;非 fail-fast 打开时该段按损坏跳过或降级重建,绝不返回错误数据 | `src/persist/msec/delta.rs::malformed_delta_is_rejected` | Passed |
...
| FC-PERSIST-STA-004 | STA | 多段 MANIFEST 提交:增量段 **append** 与 compaction 组 **replace** 均先写新 `MANIFEST.<v>` 再原子换 `current`;任意时刻活跃段集合 = 某 MANIFEST 版本所列集合;提交失败不改变活跃集合,已提交段 write-once | `tests/l5_contracts.rs::incremental_flush_appends_segments` | Passed |
...
| FC-PERSIST-POST-010 | POST | msec `delta` 区(设计 04 §2.2a)编解码往返一致:条目按 `(target, seqno)` 排序,恢复时读入并回放访问统计与关系变更;区内条目数与 CRC 校验通过;空区视为空 `Vec`(旧段兼容) | `src/persist/msec/delta.rs::delta_roundtrip_is_sorted_and_lossless`、`src/persist/msec/delta.rs::empty_delta_is_valid`、`tests/l5_contracts.rs::delta_access_and_relations_survive_reopen` | Passed |
| FC-PERSIST-POST-011 | POST | WAL 轮转与 Checkpoint:单文件达 `CompactionPolicy.wal_file_bytes` 换新文件、单批不跨文件;打开按文件序回放,`seqno ≤ watermark` 的帧跳过;Checkpoint 后已完全覆盖的旧 WAL 文件可删除,未覆盖前缀绝不丢;撕裂尾按 `FC-PERSIST-INV-005` 处理 | `tests/l5_contracts.rs::wal_rotation_splits_files_without_losing_batches`、`src/persist/store/wal_writer.rs::wal_index_parsing_accepts_canonical_names_only` | Passed |
...
| FC-MODEL-POST-004 | POST | **版本链保留**:每个 RowId 的最新版本(或最新墓碑,以维持当前可见状态)与 `tx_ms ≥ now − history_horizon` 的历史版本被 compaction 保留;仅 `tx_ms < now − horizon` 的历史版本可回收,整链(含最新)均为超期墓碑或整链逻辑过期时回收整个 RowId(RowId 永不复用);`as_of` 在窗口内不随后续写入/compaction 变化,`horizon = None`(默认)时永久保留 | `tests/l5_contracts.rs::history_horizon_reclaims_old_versions`、`tests/l5_contracts.rs::compaction_reclaims_tombstones_under_horizon`、`tests/l5_contracts.rs::compaction_preserves_snapshot_and_as_of_within_horizon` | Passed |
...
| FC-MODEL-POST-007 | POST | `RelationIndex::Both` 时段内 `relations` 区追加按 `(to, kind, from)` 排序的反向表;`edges::parse` 返回正向/反向两表;恢复后入边集合与 `Outgoing` 模式逐边一致、重开不丢边;默认 `Outgoing` 不写反向表 | `src/persist/edges.rs::edges_roundtrip_with_reverse`、`tests/l5_contracts.rs::reverse_relation_table_survives_reopen` | Passed |
...
| FC-LIFE-INV-008 | INV | **I8**:活跃段数 ≤ `(T−1)·log_r(N/B)+c`(size-tiered:段按 `row_count` 分层,同层段数 < `tier_count` 否则触发合并;死比率超线时重写该段);WAL 总量 ≤ `wal_bytes`(WAL 上界部分已随 L2 落地于 `FC-PERSIST-INV-004`) | `tests/l5_contracts.rs::compaction_bounds_segment_count`、`tests/l5_contracts.rs::auto_compaction_triggers_in_background` | Passed |
...
| FC-LIFE-POST-003 | POST | **增量段 flush**:`flush` 把 `seqno > watermark` 的槽位与自上次 flush 的访问/关系 `delta` 物化进**新段**;旧段保持活跃、不改写、不入 trash;`watermark` 推进、WAL Checkpoint;读取面跨段合并后与全量内存状态逐位一致;无新增且无 delta 时为空操作(不产段) | `tests/l5_contracts.rs::incremental_flush_appends_segments`、`tests/l5_contracts.rs::incremental_flush_does_not_rewrite_committed_segments` | Passed |
| FC-LIFE-POST-004 | POST | **访问统计攒批**:查询命中把 `RowId` 追加进内存缓冲,攒批(默认 `access_flush_interval` = 30s)合并落 WAL `TouchRow`(合并后 `access_delta` ≥ 1);崩溃最多丢一个攒批周期的访问计数,只影响遗忘速度估计、不影响记录可见性与检索正确性;显式 `touch` 仍即时 WAL 落盘 | `tests/l5_contracts.rs::access_hits_are_batched_and_flushed` | Passed |
...
| FC-LIFE-POST-008 | POST | `backup_to` 同盘优先硬链接、失败/跨盘回退逐文件复制;`BackupReport.hardlinked` 如实报告;两条路径产物均满足 `FC-PERSIST-POST-004` 可独立 `open` + `check` | `tests/l5_contracts.rs::backup_hardlinks_segments_when_possible` | Passed |
...
| FC-LIFE-STA-001 | STA | compaction 五元组 `M=(S={Idle,Running,Paused}, E={Trigger,Step,Pause,Resume,Done,Fail}, δ(Idle,Trigger)=Running, δ(Running,Pause)=Paused, δ(Paused,Resume)=Running, δ(Running,Done)=Idle, δ(Running,Fail)=Idle, s0=Idle, F={Idle})`;`stats().compaction` 反映当前态;`pause()` 在步骤边界生效、提交前中止不改动状态;非法转移(如 `Idle` 上 `Resume`)不改变状态 | `tests/l5_contracts.rs::compaction_respects_pause`、`tests/l5_contracts.rs::compaction_bounds_segment_count` | Passed |
| FC-LIFE-ERR-001 | ERR | compaction 运行期失败(ENOSPC/I/O/编码错误)→ 以 `Io`/`Corrupted` 上报并回到 `Idle`,已提交 MANIFEST 与数据不变;孤儿新段由下次启动清理;绝不静默吞错或半提交 | `tests/l5_contracts.rs::compaction_failure_keeps_state_and_returns_idle` | Passed |
...
| FC-GLOBAL-ERR-001 | ERR | 库绝不 panic;文档化例外共三类:① `filter!` 字面量(`src/lib.rs`);② async `spawn_blocking`(async 门面落地后启用);③ 槽位下标 `u32::try_from(..).expect` 三处(`src/memory/search.rs` 的候选收集、`src/memory/table/state.rs` 的 `rebuild_indexes`、`src/query/plan.rs` 的 `compile`),均由 `FC-MEM-INV-004`(槽位下标 ≤ `u32::MAX`)保证不可达 | `tests/life_contracts.rs::l1_api_smoke_never_panics` | Passed |
...
| FC-PERSIST-CPLX-011 | CPLX | 增量 flush:时间 $O(\Delta + D)$($\Delta$ = 未物化槽位数,$D$ = 自上次 flush 的访问/关系变更数),不重写已提交段;空间额外 $O(\Delta + D)$ | 待补 | Planned |
