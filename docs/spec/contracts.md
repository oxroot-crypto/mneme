...
| `IdExhausted { kind }` | `RowId`/`NsId`/`SeqNo` 的整型表示空间耗尽(恢复出近上限水位后再写入),绝不回绕复用(FC-PERSIST-ERR-012) | FC-PERSIST-INV-020、FC-PERSIST-ERR-012 | 理论不可达,防线保留 |
...
| FC-PERSIST-ERR-012 | ERR | WAL/段恢复中 `rowid = u64::MAX` 或 `ns_id = u32::MAX` 使水位 `checked_add(1)` 失败 → `Corrupted`,绝不 `+1` 溢出 panic(debug)或回绕复用 ID(release);`RowId`/`NsId`/`SeqNo` 分配器空间耗尽 → `IdExhausted`,绝不回绕/panic | `src/persist/recover/wal_replay.rs::wal_replay_rejects_max_rowid`、`src/persist/recover/wal_replay.rs::wal_replay_rejects_max_ns_id`、`src/memory/table/state.rs::id_allocators_reject_exhaustion` | Passed |
