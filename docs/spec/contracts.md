...
| FC-PERSIST-POST-010 | POST | msec `delta` 区(设计 04 §2.2a)编解码往返一致:条目按 `(target, seqno)` 排序,恢复时读入并回放访问统计与关系变更;`Access` delta 仅当 `seqno` 晚于该 RowId 最新版本行时才累加(版本行的 `access` 列是写入时刻累计快照,更晚版本已含该增量,重复累加会使计数虚高);区内条目数与 CRC 校验通过;空区视为空 `Vec`(旧段兼容) | `src/persist/msec/delta.rs::delta_roundtrip_is_sorted_and_lossless`、`src/persist/msec/delta.rs::empty_delta_is_valid`、`tests/l5_contracts.rs::delta_access_and_relations_survive_reopen`、`tests/l5_contracts.rs::access_delta_is_not_double_counted_after_later_version` | Passed |
| FC-PERSIST-POST-011 | POST | WAL 轮转与 Checkpoint:单文件达 `CompactionPolicy.wal_file_bytes` 换新文件、单批不跨文件;打开按文件序回放,`seqno ≤ watermark` 的帧跳过;Checkpoint 后已完全覆盖的旧 WAL 文件可删除(删除失败仅残留,恢复时按 `seqno ≤ watermark` 跳过),未覆盖前缀绝不丢;撕裂尾按 `FC-PERSIST-INV-005` 处理 | `tests/l5_contracts.rs::wal_rotation_splits_files_without_losing_batches`、`tests/l5_contracts.rs::checkpoint_removes_covered_wal_files`、`src/persist/store/wal_writer.rs::wal_index_parsing_accepts_canonical_names_only` | Passed |
