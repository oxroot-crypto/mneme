...
> `feature = "..."` 只在 `Cargo.toml` 声明了对应 feature 时才有意义。mneme 声明了
> `mmap`(默认开启,见 [01 §4.2](01-toolchain.md))、`quant-f16`、`async`、`fuzzing`、
> `encrypt`、`compress`、`compress-zstd` 与 `wasm`,上表仅作语法示例。
