...
- `rust-version = "1.93"`:MSRV(Minimum Supported Rust Version,最低支持版本)。
  它告诉 cargo 和用户"低于 1.93 的编译器别用"。mneme 在项目建立时按当时最新
  稳定版 − 2 选定 1.93,此后只在有明确理由时提升;实际支持范围以 `Cargo.toml`
  为准,CI 用 1.93 工具链验证该声明真实。

> 这行必须**真实**:文档里声明支持什么,CI 就必须测什么。

> **三个"版本"别混**:① `rustc 1.97.0` 是**编译器版本**(你装了哪个工具链);② `edition = "2024"`
